...
module.exports = function(sails) {
  ...
  var policyHookDef = {
    ...
    normalizePolicy: function (policy) {
      ...
      // Allow the use of the 'some' policy, intended to simulate an 'or'-like clause
      // { some: ['isAdministrator', 'isPowerUser'] }
      if (_.isObject(policy)) {
        const keys = _.keys(policy)
        const firstKey = _.first(keys);
        if (keys.length === 1 && firstKey === 'some') {
          const values = _.values(policy[firstKey]);
          const functions = values.map(policyName => {
            const policyFn = this.lookupFn(policyName, 'config.policies');
            // Set the "policy" key on the policy function to the policy name, for debugging
          ...
          return [
            async (req, res, next) => {
              const fail = { negotiate: exception => { return exception } };
              const pass = () => { return false };
              const policiesFunctions = functions.map(fn => {
                return new Promise((resolve, reject) => {
                  return resolve(fn(req, fail, pass));
                });
              })
              const policiesResults = await Promise.all(policiesFunctions);
              const failedPolicies = policiesResults.filter(result => !!result);
              const passedPolicies = policiesResults.filter(result => !result);

              const atLeastOnePassed = passedPolicies.length;
              if (atLeastOnePassed) {
                next();
              } else {
                res.negotiate(_.first(failedPolicies));
              }
            }
          ];
        }
